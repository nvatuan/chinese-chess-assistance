...
def guess(img, oneLetterMode=False, IMAGE_DIM=(50, 50)):
    ...
    return (tflite_getLabelFromOutput(np.argmax(output), oneLetterMode))

def guessFile(filename):
    img = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)
    return tflite_guess(img)

if __name__ == "__main__":
    tflite_loadInterpreter('./model.tflite')
    ...
    for idx, g in enumerate(GuessList):
        G = tflite_guessFile(GUESS_DIR + '/' + g)
        #print("I guess:", G)
